...
{
	public GameObject AvatarSphereBody;
	public GameObject AvatarSphereCollider;
	...
	public void saveSkeleton(string filename){
		...
		FileStream file = File.Create(Application.dataPath + "/StreamingAssets/Poses/" + filename + ".dat");
	...
	public bool loadSkeleton(float[] skeletonarray, String filename) {
		if (File.Exists(Application.dataPath + "/StreamingAssets/Poses/" + filename + ".dat")) {
			BinaryFormatter bf = new BinaryFormatter();
			FileStream file = File.Open(Application.dataPath + "/StreamingAssets/Poses/" + filename + ".dat", FileMode.Open);
